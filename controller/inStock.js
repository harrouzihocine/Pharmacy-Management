...
exports.serviceStockDetails = async (req, res) => {
  ...
  try {
      ...
      // Generate barcode images for each stockItem
      for (const stockItem of stockDetails) {
          stockItem.isExpiringSoon = await stockItem.getExpirationStatus();
          
          try {
              // Use existing barcode directly from InStock model
              const barcodeImage = await bwipjs.toBuffer({
                  bcid: 'code128',    // Barcode type
                  text: stockItem.barcode,  // Use existing barcode directly
              ...
              // Store original barcode value and add base64 image
              stockItem.originalBarcode = stockItem.barcode; // Save the original barcode
              stockItem.barcodeImage = `data:image/png;base64,${barcodeImage.toString('base64')}`;
          } catch (barcodeError) {
              console.error('Error generating barcode image:', barcodeError);
              stockItem.barcodeImage = null; // Set to null if generation fails
          }
      }
