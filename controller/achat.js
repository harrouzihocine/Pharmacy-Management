...
exports.createPurchaseRequest = async (req, res) => {
  try {
    ...
    const PurchaseRequestCode = `D${yearShort}/${String(
      sequenceNumber
    ).padStart(4, "0")}`;
...
exports.createFactureProforma = async (req, res) => {
  try {
    ...
    for (const fournisseurId in facturesByFournisseur) {
      ...
      const factureProformaCode = `FP${yearShort}/${String(
        sequenceNumber
      ).padStart(4, "0")}`;
...
exports.createBCFromDemand = async (req, res) => {
  try {
    ...
    for (const fournisseurId in facturesByFournisseur) {
      const medicaments = facturesByFournisseur[fournisseurId];
      const bonCommandeCode = `BC${yearDigits}/${String(
        sequenceNumber
      ).padStart(4, "0")}`;
      // Ensure medicaments is not null or undefined before applying filter
      if (!medicaments) continue;
...
exports.updateBonCommande = async (req, res) => {
  ...
  const { fournisseurId, dateBonCommande, medicaments, comment } = req.body;
  ...
  try {
    ...
    if (!fournisseurId || !dateBonCommande || !medicaments) {
      req.flash(
        "error",
        "Required inputs are missing: Fournisseur, Date, or Medicaments."
      );
      ...
      for (const item of medicaments) {
        if (item.medicamentId && item.quantity) {
          ...
          if (!medicament) {
            req.flash(
              "error",
              `Medicament with ID ${item.medicamentId} not found.`
            );
            return res.redirect(`/achat/approvisionnement/BC/edit/${BCId}`);
          }
...
exports.cancelBC = async (req, res) => {
  try {
    ...
    const bonDeReception = await BonDeReception.findOne({
      bonCommandeId: BCId,
    });
...
exports.createBonReception = async (req, res) => {
  ...
  const bonReceptionCode = `AR${lastTwoDigits}/${String(
    sequenceNumber
  ).padStart(4, "0")}`;
...
exports.getBEDetailsPage = async (req, res, next) => {
  ...
  try {
    ...
    const bonEntreeItems = await BonEntreeItem.find({
      ...
      .populate("medicamentId", "designation boite_de code_interne nom_commercial")
      .populate("inStockId", "barcode originalBarcode")
      .lean();

    // Generate QR codes for each item
    for (const item of bonEntreeItems) {
      if (item.inStockId && item.inStockId.barcode) {
        try {
          // Generate QR code
       
          const qrCodeData = await new Promise((resolve, reject) => {
            QRCode.toDataURL(item.inStockId.barcode || 'NO_BARCODE', {
              width: 300,
              height: 300,
              margin: 1,
              color: {
          ...
          // Store both the QR code and original barcode data
          item.barcode = qrCodeData;
          item.originalBarcode = item.inStockId.barcode;
        } catch (error) {
          console.error('Error generating QR code:', error);
          item.barcode = null;
          item.originalBarcode = null;
        }
      }
    }
...
exports.updateBEItem = async (req, res) => {
  try {
    ...
    if (!itemId || !medicamentId || !quantity) {
      return res
        .status(400)
...
exports.transferBEToStorage = async (req, res, next) => {
  try {
    const BeId = req.params.BEID;

    // Get BonEntree with serviceABV
    const bonEntree = await BonEntree.findById(BeId);
    if (!bonEntree) {
      return res.status(404).json({
        success: false,
    ...
    // Find all items for this BE
    const bonEntreeItems = await BonEntreeItem.find({
      bonEntreeId: BeId,
    }).populate('medicamentId');

    if (!bonEntreeItems || bonEntreeItems.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No items found in this Bon Entree",
      });
    }

    // Find storage based on serviceABV
    const storage = await Storage.findOne({
      serviceABV: bonEntree.serviceABV
    });

    if (!storage) {
      return res.status(404).json({
        success: false,
        message: `No storage found for service ${bonEntree.serviceABV}`,
      });
    }

    // Transfer each item to InStock and update BonEntreeItem
    const transferPromises = bonEntreeItems.map(async (item) => {
      // Create and save new InStock item
      const newInStockItem = new InStock({
        medicamentId: item.medicamentId._id,
        storageId: storage._id,
        locationCode: item.storageName, // Using storageName as locationCode
      ...
      const savedInStock = await newInStockItem.save();
      
      // Update BonEntreeItem with the InStock reference
      await BonEntreeItem.findByIdAndUpdate(item._id, {
        inStockId: savedInStock._id
      });
    ...
    // Update BonEntree status to indicate transfer is complete
    bonEntree.status = 'Transferred';
    await bonEntree.save();

    res.json({
      success: true,
      message: "Items successfully transferred to storage",
    });

  } catch (error) {
    console.error('Error in transferBEToStorage:', error);
    res.status(500).json({
      success: false,
      message: "An error occurred while transferring items to storage",
      error: error.message
    });
  }
};
exports.updateBarCode = async (req, res) => {
    ...
    console.log("Received BarCode:", BarCode, "for itemId:", itemId);
      try {
       
        const stockItem = await InStock.findById(itemId);
