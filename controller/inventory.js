...
module.exports.createInventory = async (req, res, next) => {
  try {
    const { title, description, serviceABV, storageName, inventoryType, workflowType } = req.body;

    // Determine if inventory is empty or pre-filled
    const isEmpty = inventoryType === 'empty';
    ...
    // If inventory is not empty, pre-populate with current stock
    if (!isEmpty) {
      await this.prePopulateInventoryItems(newInventory._id, workflowType, serviceABV, storageName, req.user._id);
    }
...
// Helper function to pre-populate inventory items based on current stock
module.exports.prePopulateInventoryItems = async (inventoryId, workflowType, serviceABV, storageName, userId) => {
  try {
    ...
    if (workflowType === 'allInOne') {
      ...
      // Get stock items for specific service
      const storages = await Storage.find({ serviceABV }).select('_id');
      ...
      // Get stock items for specific service and storage
      const storage = await Storage.findOne({ serviceABV, storageName }).select('_id');
      if (storage) {
        stockQuery = { 
    ...
    // Fetch stock items based on query
    const stockItems = await InStock.find(stockQuery)
      .populate('medicamentId')
      .populate('storageId');

    // Create inventory items from stock
    const inventoryItems = stockItems.map(stockItem => ({
      inventoryId,
      medicamentId: stockItem.medicamentId._id,
      serviceABV: stockItem.storageId.serviceABV,
      storageName: stockItem.storageId.storageName,
      batchNumber: stockItem.batchNumber,
      serialNumber: stockItem.serialNumber,
      expiryDate: stockItem.expiryDate,
      tva: stockItem.tva,
      systemQuantity: stockItem.quantity, // Set system quantity from current stock
      physicalQuantity: 0, // User will fill this during inventory
      purchasePrice: stockItem.purchase_price || 0,
      createdBy: userId,
    }));
    ...
    console.error('Error pre-populating inventory items:', error);
    throw error;
  }
};
...
module.exports.updatePhysicalQuantity = async (req, res, next) => {
  try {
    const itemId = req.params.itemId;
    const { physicalQuantity } = req.body;

    // Validate input
    if (!itemId || physicalQuantity === undefined || physicalQuantity < 0) {
    ...
    res.json({ 
      success: true, 
      message: 'Physical quantity updated successfully',
